...
export async function searchAppointment({ appointment, io }: SearchAppointmentArgs) {
  ...
  try {
    ...
    logger.debug('Appointment -> before -> const tokenData = await db.cookie.findUnique({')
    const tokenData = await db.cookie.findUnique({
      where: { userId: appointment.userId, appoinmentId: appointment.id, expiresAt: { gt: new Date() } }
    })
    ...
    if (!tokenData) {
      ...
      if (data?.token) {
        logger.debug('Appointment -> before -> const response = await db.cookie.create({')
    ...
    if (token) {
      ...
      if (!polyclinicId) return new Response(400, null, 'There is no polyclinicId')

      logger.debug('Appointment -> before -> const doctors = await getDoctors({ cityId, districtId, polyclinicId, token })')
      const doctors = await getDoctors({ cityId, districtId, polyclinicId, token })

      logger.debug('Appointment -> before -> if (!appointment.doctorId && !anyDoctor) {')
      if (!appointment.doctorId && !anyDoctor) {
        io.to(appointment.userId).emit('doctors', { appointmentId: appointment.id, doctors })
      ...
      const response = await addAppointments({ appointment, appointmentHours, token })
